...
const checkDeepLink = async () => {
    ...
    if (dateQuery) {
        ...
        if (!isNaN(dateObj.getTime())) {
            ...
            // Define o evento destacado se vier da sidebar
            if (eventId) {
                highlightedEventId.value = parseInt(eventId);
    ...
    <div class="py-8 animate-fade-in w-full min-h-screen">
        <div class="w-full">
            <h1
                ...
                Agenda<span class="hidden md:inline"> - {{ viewDate.toLocaleString('pt-BR', {
                    month: 'long', year:
                    'numeric' }) }}</span>
            </h1>
            <div class="flex flex-col lg:flex-row gap-8 items-start">
                <div class="lg:hidden w-full mb-4">
                    <div
                        class="bg-gray-900/80 backdrop-blur-md rounded-xl p-4 border border-gray-800 shadow-xl flex items-center justify-between">
                        <button @click="changeMonth(-1)" class="p-2 text-purple-400"><i
                                class="pi pi-chevron-left"></i></button>
                        <div class="text-center">
                            <div class="text-xl font-bold text-white capitalize">{{ viewDate.toLocaleString('pt-BR', {
                                month: 'long' }) }}</div>
                ...
                <div class="hidden lg:block w-full lg:w-[350px] shrink-0 lg:sticky lg:top-4 z-20">
                    <div class="bg-gray-900/80 backdrop-blur-md rounded-xl p-4 border border-gray-800 shadow-xl">
                        <Calendar :key="calendarKey" v-model="calendarDate" inline class="custom-dark-calendar w-full"
                            @date-select="onDateSelect" @month-change="onMonthChange" @year-change="onYearChange">
                        ...
                        <div v-if="selectedDate" class="mt-4 pt-4 border-t border-gray-800">
                            <button @click="selectedDate = null"
                                class="w-full py-2 text-xs font-bold text-gray-400 hover:text-white flex items-center justify-center gap-2">
                ...
                <div class="w-full flex-grow">
                    ...
                    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">
                        <div v-for="event in displayedEvents" :key="event.id" :id="`event-${event.id}`"
                            class="group bg-gray-900 rounded-xl overflow-hidden transition-all flex flex-row sm:flex-col shadow-lg"
                            :class="{
                                'border-2 border-purple-400 shadow-purple-800/50': highlightedEventId === event.id,
                                'border border-gray-800 hover:border-purple-500': highlightedEventId !== event.id
                            }">
                            <div class="relative w-28 sm:w-full sm:aspect-[2/3] shrink-0 overflow-hidden">
                                ...
                                <div
                                    class="absolute top-1 right-1 sm:top-3 sm:right-3 bg-black/80 border border-purple-500/50 text-white p-1 sm:p-2 text-center rounded-md sm:rounded-lg">
                                    <div class="text-sm sm:text-xl font-black text-purple-400">{{ event.day || new
                                        Date(event.date ||
                                        event.event_date).getDate() }}</div>
                            ...
                            <div class="p-3 sm:p-5 flex flex-col flex-grow justify-between">
                                ...
                                <a v-if="event.ticket_url" :href="event.ticket_url" target="_blank"
                                    class="mt-2 sm:mt-4 py-1.5 bg-white/5 hover:bg-purple-600 text-center rounded-lg text-xs font-bold transition-colors">Ingressos</a>
                            </div>
                        </div>
                    </div>
                </div>
